...
namespace OpenLiveWriter.BlogClient.Clients
{
    ...
    public abstract class XmlRpcBlogClient : BlogClientBase, IBlogClient
    {
        ...
        public abstract Task<BlogInfo[]> GetUsersBlogsAsync();
        ...
        public abstract Task<BlogPostCategory[]> GetCategories(string blogId);
        public abstract Task<BlogPostKeyword[]> GetKeywords(string blogId);

        public abstract Task<BlogPost[]> GetRecentPostsAsync(string blogId, int maxPosts, bool includeCategories, DateTime? now);
        ...
        public abstract Task<bool> EditPost(string blogId, BlogPost post, INewCategoryContext newCategoryContext, bool publish);

        public abstract Task<BlogPost> GetPost(string blogId, string postId);

        public abstract Task DeletePost(string blogId, string postId, bool publish);
        ...
        public Task<string> NewPage(string blogId, BlogPost page, bool publish, string etag, XmlDocument remotePost)
        {
            etag = null;
            remotePost = null;
        ...
        protected virtual Task<string> NewPage(string blogId, BlogPost page, bool publish)
        {
            throw new BlogClientMethodUnsupportedException("NewPage");
        }

        public Task<bool> EditPage(string blogId, BlogPost page, bool publish, string etag, XmlDocument remotePost)
        {
            etag = null;
            remotePost = null;
        ...
        protected virtual Task<bool> EditPage(string blogId, BlogPost page, bool publish)
        {
        ...
        public abstract Task<string> DoBeforePublishUploadWork(IFileUploadContext uploadContext);
        ...
        protected virtual Task BeforeHttpRequest(HttpRequestMessage request)
        {
        ...
        public virtual Task<string> AddCategory(string blogId, BlogPostCategory category)
        {
        ...
        /// <returns></returns>
        protected async Task<IXmlNode> CallMethod(string methodName, params XmlRpcValue[] parameters)
        {
            ...
            {
                // create an RpcClient
                XmlRpcClient xmlRpcClient = new XmlRpcClient(url, ApplicationEnvironment.UserAgent, new HttpAsyncRequestFilter(BeforeHttpRequest), _clientOptions.CharacterSet);

                // call the method
                XmlRpcMethodResponse response = await xmlRpcClient.CallMethod(methodName, parameters);
        ...
        protected static string NodeText(IXmlNode node)
        {
            if (node != null)
        ...
        /// <returns></returns>
        protected virtual DateTime ParseBlogDate(IXmlNode xmlNode)
        {
            ...
            if (date != String.Empty)
            {
                ...
                {
                    //Debug.Fail("Unexpected exception parsing date: " + date + " (" + ex.Message + ")");
