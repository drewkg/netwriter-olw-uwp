...
namespace OpenLiveWriter.CoreServices
{
    ...
    public class UrlHelper
    {
        ...
        public static string CreateUrlFromPath(string path)
        {
			...
			if ( result == HRESULT.S_OK )
			{
				...
				try
				{
					...
					{
						Debug.Fail("Possible bug encoding/decoding path: " + path);
					}
				}
				catch(Exception ex)
				{
					Debug.Fail("Exception while checking path encoding. Original path: " + path + " url from Shlwapi: " + url);
					throw ex;
				}
				return url;
			}
        ...
        private static string ShlwapiFileUrlToDotnetEncodedUrl(string s)
        {
            ...
            string encodedUrl = FILE_SCHEME + WebUtility.UrlEncode(decodedUrl);
            ...
            if (containsUnsafeChars) //fixes bug 482444
            {
                foreach (char unsafeChar in UNSAFE_URL_CHARS)
                {
                    ...
                    encodedUrl = encodedUrl.Replace(unsafeCharStr, WebUtility.UrlEncode(unsafeCharStr)); //URLPathEncode doesn't encode # signs.
                }
            }
            return encodedUrl;
        }
        ...
        /// </summary>
        private static string[] KnownSchemes = new string[] {	"file", "http", "ftp", "gopher", "https", "mailto", "news", "nntp", "telnet", "wais", "ldap" };        
        ...
        public static string GetBaseUrl(string url)
        {
            ...
            catch (Exception e)
            {
                //if (!url.StartsWith("outbind", StringComparison.OrdinalIgnoreCase)) // Outlook URLs (pre-2007) don't have base URLs
                    //Debug.Fail("Failed to parse URI: " + url + "\r\n" + e.ToString());
                return string.Empty;
            }
        }
        ...
        public static string GenerateShortcutFileForUrl(string fileName, string url)
        {
            ...
            string shortcutFile = System.IO.Path.ChangeExtension(Path.Combine(System.IO.Path.GetTempPath(), fileName), ".url");
        ...
        public static string UrlCombine(string baseUrl, string relativeUrl)
        {
            ...
            if (!baseUrl.StartsWith("mhtml:file://", StringComparison.OrdinalIgnoreCase))
            {
                ...
                // double escaped.
                if (IsUrl(baseUrl))
                    baseUrl = WebUtility.UrlDecode(baseUrl);

                //relativeUrl =  Shlwapi.UrlCombine(
                //    baseUrl,
                //    relativeUrl);
                relativeUrl = new Uri(new Uri(baseUrl), relativeUrl).ToString();
            }
            else
            {
        ...
        private static string[] NonlinkableSchemes = new string[] { "file" };
        ...
        private static string[] DownloadableSchemes = new string[] { "file","http","https" };
