...
namespace OpenLiveWriter.BlogClient.Clients
{
	internal class AtomEntry
	{
		...
		{
			return _entryNode.SelectSingleNodeNS(xpath, _nsMgr.ToNSMethodFormat()) as XmlElement;
		}
		...
		private IXmlNode PopulateElement(string val, Namespace ns, string localName)
		{
			RemoveNodes(localName, ns);

			if (val != null)
			{
				var newNode = _entryNode.OwnerDocument.CreateElementNS(ns.Uri, ns.Prefix + ":" + localName);
...
$", RegexOptions.IgnorePatternWhitespace);
		...
		{
		    TimeSpan offset = TimeZoneInfo.Utc.GetUtcOffset(dateTime);
		...
		{
			if (_entryNode.SelectSingleNodeNS("atom:id", _nsMgr.ToNSMethodFormat()) == null)
				PopulateElement("urn:uuid:" + Guid.NewGuid().ToString("d"), _atomNS, "id");
		}

		public void GenerateUpdated()
		{
			if (_entryNode.SelectSingleNodeNS("atom:updated", _nsMgr.ToNSMethodFormat()) == null)
