	...
	public class FormData
	{
		...
		public override string ToString()
		{
			...
			for (int i = 0; i < pairs.Count; i++)
			{
				...
				output.Append(WebUtility.UrlEncode(pair.Name));
				if (pair.Value != null)
				{
					output.Append("=");
					output.Append(WebUtility.UrlEncode(pair.Value));
				}
			}
		...
		public Stream ToStream()
		{
			...
			for (int i = 0; i < pairs.Count; i++)
			{
				...
				NameValuePair pair = (NameValuePair)pairs[i];
			    var bytes = GetBytes(pair.Name);
				WriteBytes(stream, WebUtility.UrlEncodeToBytes(bytes, 0, bytes.Length));
				if (pair.Value != null)
				{
					stream.WriteByte((byte)'=');
                    var bytesValue = GetBytes(pair.Value);
                    WriteBytes(stream, WebUtility.UrlEncodeToBytes(bytesValue, 0, bytesValue.Length));
				}
			}
