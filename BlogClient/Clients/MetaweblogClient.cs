...
namespace OpenLiveWriter.BlogClient.Clients
{
    ...
    public class MetaweblogClient : BloggerCompatibleClient
    {
        ...
        private async Task<BlogPostKeyword[]> WordPressGetKeywords(string blogId)
        {
            // call the method
            var result = await CallMethod("wp.getTags",
            ...
            try
            {
                var keywordNodes = result.SelectNodes("array/data/value/struct");
            ...
            catch (Exception ex)
            {
                ...
                //Debug.Fail("Exception occurred while parsing GetKeywords response: " + response + "\r\n" + ex.ToString());
        ...
        private async Task<BlogPostCategory[]> MetaweblogGetCategories(string blogId)
        {
            // call the method
            var result = await CallMethod("metaWeblog.getCategories",
                new XmlRpcString(blogId),
                new XmlRpcString(Username),
                new XmlRpcString(Password, true));
        ...
        private Task<BlogPostCategory[]> MovableTypeGetCategories(string blogId)
        {
            return EnhancedGetCategories("mt.getCategoryList", blogId, false);
        }

        private Task<BlogPostCategory[]> WordPressGetCategories(string blogId)
        {
            return EnhancedGetCategories("wp.getCategories", blogId, true);
        }

        private async Task<BlogPostCategory[]> EnhancedGetCategories(string methodName, string blogId, bool allowParent)
        {
            // call the method
            var result = await CallMethod(methodName,
                new XmlRpcString(blogId),
                new XmlRpcString(Username),
                new XmlRpcString(Password, true));
            ...
            try
            {
                var categoryNodes = result.SelectNodes("array/data/value/struct");
            ...
            {
                string response = result != null ? result.GetXml() : "(empty response)";
                //Debug.Fail("Exception occurred while parsing GetCategories response: " + response + "\r\n" + ex.ToString());
        ...
        public virtual BlogPostCategory ParseCategory(IXmlNode categoryNode)
        {
            // parse out the category name
        ...
        private string GetNodeValue(IXmlNode xmlNode, string xpath)
        {
            var nodeValue = xmlNode.SelectSingleNode(xpath);
        ...
        {
            // call the method	
            var result = await CallMethod("metaWeblog.getRecentPosts",
                new XmlRpcString(blogId),
                new XmlRpcString(Username),
                new XmlRpcString(Password, true),
            ...
            // parse results
            return await ParsePosts(result, "metaWeblog.getRecentPosts", includeCategories, now);
        }
        ...
        public override async Task<string> NewPost(string blogId, BlogPost post, INewCategoryContext newCategoryContext, bool publish)
        {
            ...
            bool addCategoriesOutOfBand = await AddCategoriesIfNecessary(blogId, post, newCategoryContext);

            string result;
            if (Options.SupportsCategoriesInline)
            {
                result = await MetaweblogNewPost(blogId, post, publish);
                if (Options.SupportsHierarchicalCategories)
                {
                    await MovableTypeSetPostCategories(result, ArrayHelper.Concat(post.Categories, post.NewCategories));
                }
            }
            else
            {
                result = await MovableTypeNewPost(blogId, post, publish);
            }
        ...
        private async Task<bool> AddCategoriesIfNecessary(string blogId, BlogPost post, INewCategoryContext newCategoryContext)
        {
            ...
            foreach (BlogPostCategory category in post.NewCategories)
            {
                string categoryId = await AddCategory(blogId, category);
        ...
        private async Task<string> MetaweblogNewPost(string blogId, BlogPost post, bool publish)
        {
            // call the method
            var result = await CallMethod("metaWeblog.newPost",
                                         new XmlRpcString(blogId),
                                         new XmlRpcString(Username),
                                         new XmlRpcString(Password, true),
        ...
        private async Task<string> MovableTypeNewPost(string blogId, BlogPost post, bool publish)
        {
            ...
            string postId = await MetaweblogNewPost(blogId, post, false);

            // add the categories to the post
            await MovableTypeSetPostCategories(postId, post.Categories);

            if (publish)
            {
                // call the metaweblog edit post for publishing
                post.Id = postId;
                await MetaweblogEditPost(blogId, post, publish);
            }
        ...
        private async Task MovableTypeSetPostCategories(string postId, BlogPostCategory[] categories)
        {
            //if (categories == null || categories.Length == 0) return;

            // call method
            var result = await CallMethod("mt.setPostCategories",
            ...
            Debug.Assert(result.InnerText.Trim() == "1", "Unexpected error return value from mt.setPostCategories " + result.GetXml());
        }
        ...
        public override async Task<bool> EditPost(string blogId, BlogPost post, INewCategoryContext newCategoryContext, bool publish)
        {
            ...
            if (Options.SupportsCategoriesInline)
            {
                result = await MetaweblogEditPost(blogId, post, publish);
                if (Options.SupportsHierarchicalCategories)
                    await MovableTypeSetPostCategories(post.Id, ArrayHelper.Concat(post.Categories, post.NewCategories));
            }
            else
            {
                result = await MovableTypeEditPost(blogId, post, publish);
            }
        ...
        private async Task<bool> MetaweblogEditPost(string blogId, BlogPost post, bool publish)
        {
            // call the method
            var result = await CallMethod("metaWeblog.editPost",
        ...
        private async Task<bool> MovableTypeEditPost(string blogId, BlogPost post, bool publish)
        {
            // add the categories to the post
            await MovableTypeSetPostCategories(post.Id, post.Categories);

            // call the base to do the rest of the edit
            return await MetaweblogEditPost(blogId, post, publish);
        }
        ...
        {
            // call method
            var result = await CallMethod("metaWeblog.getPost",
                new XmlRpcString(postId),
                new XmlRpcString(Username),
                new XmlRpcString(Password, true));
            ...
            var postStruct = result.SelectSingleNode("struct");
            if (postStruct != null)
            {
                return await ParseBlogPost(postStruct, true);
            }
            else
            {
                throw new BlogClientInvalidServerResponseException("metaWeblog.getPost", "No post struct returned from server", result.GetXml());
            }
        }

        public override async Task<BlogPost> GetPage(string blogId, string pageId)
        {
            if (Options.SupportsPages)
            {
                // call method
                var result = await CallMethod("wp.getPage",
                    new XmlRpcString(blogId),
                ...
                if (postStruct != null)
                {
                    ...
                    return await ParseBlogPost(postStruct, false);
                }
                else
                {
                    throw new BlogClientInvalidServerResponseException("wp.getPage", "No post struct returned from server", result.GetXml());
                }
            }
            else
            {
        ...
        public override async Task<BlogPost[]> GetPages(string blogId, int maxPages)
        {
            if (Options.SupportsPages)
            {
                // call the method	
                var result = await CallMethod("wp.getPages",
                    new XmlRpcString(blogId),
                    new XmlRpcString(Username),
                    new XmlRpcString(Password, true),
                ...
                // parse results
                return await ParsePosts(result, "wp.getPages", false, DateTime.Now);
            }
            else
            {
        ...
        public override async Task<PageInfo[]> GetPageList(string blogId)
        {
            if (Options.SupportsPages)
            {
                // call the method
                var result = await CallMethod("wp.getPageList",
                    new XmlRpcString(blogId),
                    new XmlRpcString(Username),
                    new XmlRpcString(Password, true));
                ...
                try
                {
                    var pageNodes = result.SelectNodes("array/data/value/struct");
                    foreach (var pageNode in pageNodes)
                    {
                        // page id
                        string pageId = GetNodeValue(pageNode, "member[name='page_id']/value");
                        if (pageId == null)
                            throw new BlogClientInvalidServerResponseException("wp.getPageInfo", "page_id member of struct not supplied", result.GetXml());

                        // page title
                        string pageTitle = GetNodeValue(pageNode, "member[name='page_title']/value");
                        if (pageTitle == null)
                            throw new BlogClientInvalidServerResponseException("wp.getPageInfo", "page_title member of struct not supplied", result.GetXml());
                ...
                {
                    string response = result != null ? result.GetXml() : "(empty response)";
                    //Debug.Fail("Exception occurred while parsing GetPageList response: " + response + "\r\n" + ex.ToString());
        ...
        protected override async Task<string> NewPage(string blogId, BlogPost page, bool publish)
        {
            if (Options.SupportsPages)
            {
                // call the method
                var result = await CallMethod("wp.newPage",
                    new XmlRpcString(blogId),
                    new XmlRpcString(Username),
                    new XmlRpcString(Password, true),
        ...
        protected override async Task<bool> EditPage(string blogId, BlogPost page, bool publish)
        {
            if (Options.SupportsPages)
            {
                // call the method
                var result = await CallMethod("wp.editPage",
                    new XmlRpcString(blogId),
        ...
        public override async Task DeletePage(string blogId, string pageId)
        {
            if (Options.SupportsPages)
            {
                var result = await CallMethod("wp.deletePage",
                    new XmlRpcString(blogId),
                    new XmlRpcString(Username),
                    new XmlRpcString(Password, true),
                    new XmlRpcString(pageId));
        ...
        public override async Task<AuthorInfo[]> GetAuthors(string blogId)
        {
            if (Options.SupportsAuthor)
            {
                // call the method
                var result = await CallMethod("wp.getAuthors",
                    new XmlRpcString(blogId),
                    new XmlRpcString(Username),
                    new XmlRpcString(Password, true));
                ...
                try
                {
                    var authorNodes = result.SelectNodes("array/data/value/struct");
                    foreach (var authorNode in authorNodes)
                    {
                        // page id
                        string authorId = GetNodeValue(authorNode, "member[name='user_id']/value");
                        if (authorId == null)
                            throw new BlogClientInvalidServerResponseException("wp.getAuthors", "user_id member of struct not supplied", result.GetXml());

                        // page name
                        string authorName = GetNodeValue(authorNode, "member[name='display_name']/value");
                        if (authorName == null)
                            throw new BlogClientInvalidServerResponseException("wp.getAuthors", "user_login member of struct not supplied", result.GetXml());
                ...
                {
                    string response = result != null ? result.GetXml() : "(empty response)";
                    //Debug.Fail("Exception occurred while parsing GetAuthors response: " + response + "\r\n" + ex.ToString());
        ...
        public override async Task<string> DoBeforePublishUploadWork(IFileUploadContext uploadContext)
        {
            ...
            {
                result = await CallMethod("metaWeblog.newMediaObject",
            ...
            var urlNode = result.SelectSingleNode("struct/member[name='url']/value");
            if (urlNode != null)
            {
            ...
            {
                throw new BlogClientInvalidServerResponseException("metaWeblog.newMediaObject", "No URL returned from server", result.GetXml());
            }
        }
        ...
        public override async Task<string> AddCategory(string blogId, BlogPostCategory category)
        {
            // call the method
            var result = await CallMethod("wp.newCategory",
                new XmlRpcString(blogId),
                new XmlRpcString(Username),
                new XmlRpcString(Password, true),
                new XmlRpcStruct(new XmlRpcMember[]
        ...
        private static int ParseCategoryParent(string parent)
        {
            ...
            if (!string.IsNullOrEmpty(parent))
            {
                ...
                {
                    //Debug.Fail("Unexpected non-integer category parent ID: " + parent);
                }
            }
            return val;
        }

        public override async Task<BlogPostCategory[]> SuggestCategories(string blogId, string partialCategoryName)
        {
            // call the method
            var result = await CallMethod("wp.suggestCategories",
                new XmlRpcString(blogId),
                new XmlRpcString(Username),
                new XmlRpcString(Password, true),
        ...
        private BlogPostCategory[] ParseCategories(IXmlNode result, string methodName)
        {
            // parse the results			
            ArrayList categories = new ArrayList();
            try
            ...
            {
                string response = result != null ? result.GetXml() : "(empty response)";
                //Debug.Fail("Exception occurred while parsing " + methodName + " response: " + response + "\r\n" + ex.ToString());
                throw new BlogClientInvalidServerResponseException(methodName, ex.Message, response);
            }
                    ...
                    else
                    {
                        switch (post.CommentPolicy)
                        {
                            ...
                            case BlogCommentPolicy.Closed:
                                ...
                                break;
                            default:
                                Debug.Fail("Unexpected blog comment policy: " + post.CommentPolicy.ToString());
                                break;
                        }
                    }
        ...
        protected virtual void GeneratePostDateFields(BlogPost post, ArrayList members)
        {
            ...
            else if (Options.AlwaysSendPostDate)
            {
                DateTime date = post.HasDatePublishedOverride ? post.DatePublishedOverride : post.DatePublished;
        ...
        private async Task<BlogPost[]> ParsePosts(IXmlNode result, string methodName, bool includeCategories, DateTime? now)
        {
            // parse results
            ...
            try
            {
                ...
                {
                    BlogPost blogPost = await ParseBlogPost(postNode, includeCategories);
        ...
        private async Task<BlogPost> ParseBlogPost(IXmlNode postNode, bool includeCategories)
        {
            // create blog post 
            ...
            // date published
            var dateCreatedNode = postNode.SelectSingleNode("member[name='date_created_gmt']/value");
                ...
                if (commentPolicy != String.Empty)
                {
                    switch (commentPolicy)
                    {
                        ...
                        case "2":
                            ...
                            break;
                        default:
                            //Debug.Fail("Unexpected value for mt_allow_comments: " + commentPolicy);
                            break;
                    }
                }
            }
            ...
            if (Options.SupportsTrackbacks)
            {
                ...
                var pingUrlsNode = postNode.SelectSingleNode("member[name='mt_tb_ping_urls']/value");
                if (pingUrlsNode != null)
                {
                    ArrayList sentPingUrls = new ArrayList();
                    if (Options.TrackbackDelimiter == TrackbackDelimiter.ArrayElement)
                    {
                        var pingUrlNodes = pingUrlsNode.SelectNodes("array/data/value");
                        foreach (var node in pingUrlNodes)
                    ...
                    else
                    {
                        ...
                        if (Options.TrackbackDelimiter == TrackbackDelimiter.Space)
                            ...
                            delimiter = ",";
                        sentPingUrls.AddRange(pingUrlsNode.InnerText.Split(new [] { delimiter}, StringSplitOptions.None));
                    }
            ...
            if (includeCategories && Options.SupportsCategories)
            {
                ...
                {
                    blogPost.Categories = await MovableTypeExtractCategories(blogPost.Id, postNode);
                }
            }

            if (Options.SupportsPostAsDraft)
            {
                var publishNode = postNode.SelectSingleNode("member[name='publish']/value");
                if (publishNode != null)
                {
                    blogPost.IsPublished = NodeText(publishNode) == "1";
                }
                else
                {
                    publishNode = postNode.SelectSingleNode("member[name='post_status']/value");
                    if (publishNode != null)
                    {
                        blogPost.IsPublished = NodeText(publishNode) == "publish";
                    }
                    else
                    {
                        // fall back to assuming its published
        ...
        private BlogPostCategory[] MetaweblogExtractCategories(string postId, IXmlNode postNode)
        {
            // extract categories from the post
            ArrayList categories = new ArrayList();
            var categoriesNode = postNode.SelectSingleNode("member[name='categories']/value");
            if (categoriesNode != null)
            {
                var categoryNodes = categoriesNode.SelectNodes("array/data/value");
                foreach (var categoryNode in categoryNodes)
                {
                    categories.Add(
        ...
        private async Task<BlogPostCategory[]> MovableTypeExtractCategories(string postId, IXmlNode postNode)
        {
            // call the method
            var result = await CallMethod("mt.getPostCategories",
                new XmlRpcString(postId),
                new XmlRpcString(Username),
                new XmlRpcString(Password, true));
            ...
            if (categoryNodes != null)
            {
                ...
                {
                    // parse out the category name
                    var categoryId = categoryNode.SelectSingleNode("member[name='categoryId']/value");
                    var categoryTitle = categoryNode.SelectSingleNode("member[name='categoryName']/value");
            ...
            {
                throw new BlogClientInvalidServerResponseException("mt.getPostCategories", "No categories returned from server", result.GetXml());
            }
        }
